...
{
    // Emulate old code's PWM protocol:
    // Old code used ledcWrite(pin, 90) on a 10-bit timer at 50Hz for center.
    // 90 / 1024 * 20000us = 1757.8us center.
    // Range was +/- 20 duty cycle = +/- 390.6us.
    return (int16_t)(1758 + (int32_t)axis_value * 391 / 32767);
}
...
{
    pulse_us = constrain(pulse_us, 1000, 2500);
    uint32_t duty = us_to_duty((unsigned int)pulse_us);
    
    // Convert pin to the LEDC channel initialized in setup
    if (pin == LEFT_MOTOR_PIN) {
        ledcWrite(0, duty);
    } else if (pin == RIGHT_MOTOR_PIN) {
        ledcWrite(1, duty);
    }
}
...
{
    motor_write(LEFT_MOTOR_PIN, 1758);
    motor_write(RIGHT_MOTOR_PIN, 1758);
}
...
static void send_heartbeat(void)
{
    ...
    const uint8_t *target_mac = s_have_dongle ? s_dongle_mac : bcast;
    ensure_peer(target_mac);
    esp_now_send(target_mac, (uint8_t *)&hb, sizeof(hb));
}
...
static void on_recv(const uint8_t *mac, const uint8_t *data, int len)
{
    ...
    case MC_MSG_JOYSTICK: {
        ...
        // Tank drive to match old robot code
        int32_t left = -jp.axis_ly; // Joystick Y is negative when pushed up
        int32_t right = -jp.axis_ry;
        
        // Deadband
        if (abs(left) < 2000) left = 0;
        if (abs(right) < 2000) right = 0;
...
void minicore_robot_loop(void)
{
    ...
    if (!s_enabled) {
        stop_motors();
    } else if ((now - s_last_joystick_ms) > MC_MOTOR_TIMEOUT_MS) {
        stop_motors();
    }
