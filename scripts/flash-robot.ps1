...
Write-Host "[info] Uploading main.py + minibot.py + minibot_config.py + display.py + ssd1306.py + neopixel_ring.py + button.py"
$UploadTries = 5
$uploaded = $false
for ($try = 1; $try -le $UploadTries; $try++) {
  Invoke-Tool $PyBin ($MpremoteArgs + $MprDev + @('fs', 'cp', (Join-Path $Proj 'minibot.py'), (Join-Path $Proj 'minibot_config.py'), (Join-Path $Proj 'display.py'), (Join-Path $Proj 'ssd1306.py'), (Join-Path $Proj 'neopixel_ring.py'), (Join-Path $Proj 'button.py'), (Join-Path $Proj 'main.py'), ':'))
