...
Write-Host "[info] Uploading Python files from $Proj"
$UploadTries = 5
$uploaded = $false
for ($try = 1; $try -le $UploadTries; $try++) {
  $PyFiles = @(Get-ChildItem -Path (Join-Path $Proj '*.py') -File | Where-Object { $_.Name -ne 'boot.py' } | Sort-Object Name | ForEach-Object { $_.FullName })
  Invoke-Tool $PyBin ($MpremoteArgs + $MprDev + @('fs', 'cp') + $PyFiles + @(':'))
